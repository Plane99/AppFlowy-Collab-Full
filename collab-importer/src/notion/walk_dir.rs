...
fn is_matching_notion_entry(
  ...
  match (a_id, b_id) {
    ...
    _ => normalize_notion_name(a_name) == normalize_notion_name(b_name),
  }
}

fn find_matching_directory(parent_path: &Path, name: &str, id: Option<&str>) -> Option<PathBuf> {
  let direct = parent_path.join(name);
  if direct.is_dir() {
    return Some(direct);
  }

  for entry in walk_sub_dir(parent_path) {
    if entry.path().is_dir() {
      if let Ok((dir_name, dir_id)) = name_and_id_from_path(entry.path()) {
        if is_matching_notion_entry(name, id, &dir_name, dir_id.as_deref()) {
...
fn find_matching_markdown_file(
  parent_path: &Path,
  name: &str,
  id: Option<&str>,
) -> Option<(PathBuf, Option<String>)> {
  let direct = parent_path.join(format!("{}.md", name));
  if direct.is_file() {
    if let Ok((file_name, file_id)) = name_and_id_from_path(&direct) {
      if is_matching_notion_entry(name, id, &file_name, file_id.as_deref()) {
        return Some((direct, file_id));
      }
    }
  }
  ...
  for entry in walk_sub_dir(parent_path) {
    ...
    {
      if let Ok((file_name, file_id)) = name_and_id_from_path(entry.path()) {
        if is_matching_notion_entry(name, id, &file_name, file_id.as_deref()) {
          return Some((entry.path().to_path_buf(), file_id));
        }
      }
    }
  }
...
fn find_matching_all_csv_file(
  parent_path: &Path,
  name: &str,
  id: Option<&str>,
) -> Option<(PathBuf, Option<String>)> {
  let direct = parent_path.join(format!("{}_all.csv", name));
  if direct.is_file() {
    if let Ok((csv_name, csv_id)) = name_and_id_from_path(&direct) {
      if is_matching_notion_entry(name, id, &csv_name, csv_id.as_deref()) {
        return Some((direct, csv_id));
      }
    }
  }
  ...
  for entry in walk_sub_dir(parent_path) {
    if entry.path().is_file() {
      ...
      if !file_name.to_ascii_lowercase().ends_with("_all.csv") {
        continue;
      }
      if let Ok((csv_name, csv_id)) = name_and_id_from_path(entry.path()) {
        if is_matching_notion_entry(name, id, &csv_name, csv_id.as_deref()) {
          return Some((entry.path().to_path_buf(), csv_id));
        }
      }
    }
  }
...
fn find_matching_partial_csv_file(parent_path: &Path, name: &str, id: Option<&str>) -> Option<PathBuf> {
  let direct = parent_path.join(format!("{}.csv", name));
  if direct.is_file() {
    if let Ok((csv_name, csv_id)) = name_and_id_from_path(&direct) {
      if is_matching_notion_entry(name, id, &csv_name, csv_id.as_deref()) {
        return Some(direct);
  ...
  for entry in walk_sub_dir(parent_path) {
    if entry.path().is_file() {
      ...
      if !file_name.ends_with(".csv") || file_name.ends_with("_all.csv") {
        continue;
      }
      if let Ok((csv_name, csv_id)) = name_and_id_from_path(entry.path()) {
        if is_matching_notion_entry(name, id, &csv_name, csv_id.as_deref()) {
...
pub(crate) fn process_entry(
  ...
  if ext.is_file() {
    ...
    if let Some((md_file_path, md_id)) =
    ...
    } else if let Some((all_csv_file_path, csv_id)) =
      find_matching_all_csv_file(parent_path, &name, id.as_deref())
    {
      let id = id.or(csv_id);
      let csv_file_path = find_matching_partial_csv_file(parent_path, &name, id.as_deref())
...
fn process_csv_dir(
  ...
  if csv_dir.exists() {
    let csv_source_path = if all_csv_file_path.exists() {
      all_csv_file_path
    ...
    for sub_entry in walk_sub_dir(&csv_dir) {
      ...
      // Collect resources from every sub entry (including files directly under the database directory).
...
fn process_csv_file(
  ...
  if let Some(parent) = path.parent() {
    if find_matching_directory(parent, &name, id.as_deref()).is_some() {
      return None;
    }
  }
...
#[cfg(test)]
mod notion_entry_pairing_tests {
  use super::*;

  #[test]
  fn test_dir_without_id_pairs_with_md_with_id() {
    let temp_dir = tempfile::tempdir().unwrap();
    let root = temp_dir.path();

    let page_name = "No 03_Plastic";
    let page_id = "e84963fb422d4335adde4fc9f4107f0c";
    let md_file_name = format!("{} {}.md", page_name, page_id);
    let dir_path = root.join(page_name);
    ...
    let notion_export = NotionExportContext {
      csv_relation: crate::notion::CSVRelation::default(),
      no_subpages: false,
    };
    ...
    let dir_entry = WalkDir::new(root)
      ...
      .unwrap();
    let md_entry = WalkDir::new(root)
      .max_depth(1)
      .into_iter()
      .filter_map(|e| e.ok())
      .find(|e| e.path() == md_path)
      .unwrap();

    let from_dir = process_entry("host", "workspace", &dir_entry, false, &notion_export).unwrap();
    assert_eq!(from_dir.notion_name, page_name);
    assert_eq!(from_dir.notion_id.as_deref(), Some(page_id));
    assert!(from_dir.notion_file.is_markdown());

    let from_md = process_entry("host", "workspace", &md_entry, false, &notion_export);
    assert!(from_md.is_none());
  }
}
