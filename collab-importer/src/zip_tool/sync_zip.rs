...
pub fn sync_unzip(
  ...
  for i in 0..archive.len() {
    ...
    let output_path = out_dir.join(sanitize_file_path(&filename));
    if entry.is_dir() {
      ...
      match OpenOptions::new()
        ...
        Ok(mut outfile) => {
          ...
          // Only attempt this for entries at the archive root. This avoids
          // mistakenly treating zip *attachments* inside Notion exports as
          // multi-part archives.
          if !filename.contains('/') && buffer.len() >= 4 {
