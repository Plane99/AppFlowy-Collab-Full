...
async fn assert_project_and_task(root_view: &NotionPage, include_sub_dir: bool) {
  ...
  let tasks = linked_views
    .iter()
    .find(|v| v.notion_name == "Tasks")
    .expect("missing Tasks linked view");
  let projects = linked_views
    .iter()
    .find(|v| v.notion_name == "Projects")
    .expect("missing Projects linked view");

  check_task_database(tasks).await;
  check_project_database(projects, include_sub_dir).await;
}
  ...
  for view in first_level_views {
    ...
    match view.name.as_str() {
      ...
      "Root" => {
        ...
        verify_root_second_level_views(&second_level_views, folder, uid);
      },
